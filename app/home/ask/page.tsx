...
function MemoryGame({ creature, palette, palName, onComplete }: { creature: string; palette: any; palName: string; onComplete: (score: number) => void }) {
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      <p style={{ color: 'rgba(255,255,255,.6)', fontSize: 13, marginBottom: 16 }}>{palName} te défie! Trouve toutes les paires 🃏 · {moves} coups</p>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, maxWidth: 320, margin: '0 auto' }}>
        {cards.map(card => {
          ...
          return (
            <div key={card.id} onClick={() => flip(card.id)} style={{ width: '100%', aspectRatio: '1', borderRadius: 12, background: isVisible ? palette.main : 'rgba(255,255,255,.1)', border: `2px solid ${isVisible ? palette.accent : 'rgba(255,255,255,.2)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, cursor: 'pointer', transition: 'all .2s', opacity: matched.includes(card.id) ? .5 : 1 }}>
...
function DodgeGame({ creature, palette, palName, onComplete }: { creature: string; palette: any; palName: string; onComplete: (score: number) => void }) {
  ...
  const frameRef = useRef<any>(null); const obsRef = useRef(obstacles); const palYRef = useRef(palY); const scoreRef = useRef(0)
  obsRef.current = obstacles; palYRef.current = palY
  const jump = useCallback(() => {
    if (!started) { setStarted(true); return }
    if (!alive) return
    setPalY(20); setTimeout(() => setPalY(60), 500)
  }, [alive, started])
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const i = setInterval(() => { setObstacles(prev => [...prev, { x: 100, emoji: theme.obstacles[Math.floor(Math.random() * theme.obstacles.length)], id: Date.now() }]) }, 1800)
    ...
    frameRef.current = setInterval(() => {
      setObstacles(prev => {
        ...
        for (const obs of updated) {
          if (obs.x < 18 && obs.x > 5 && palYRef.current > 45) { setAlive(false); clearInterval(frameRef.current); setTimeout(() => onComplete(scoreRef.current), 1500); return updated }
        }
        return updated
      })
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      <p style={{ color: 'rgba(255,255,255,.6)', fontSize: 13, marginBottom: 8 }}>{!started ? 'Appuie pour commencer!' : `Score: ${score}`}</p>
      <div onClick={jump} style={{ width: '100%', maxWidth: 360, height: 120, margin: '0 auto', background: theme.bg, borderRadius: 16, position: 'relative', overflow: 'hidden', cursor: 'pointer', border: `2px solid ${palette.main}` }}>
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 28, background: theme.ground, borderRadius: '0 0 14px 14px' }} />
        <div style={{ position: 'absolute', left: '8%', fontSize: 28, transition: 'bottom .15s ease', bottom: `${100 - palY}%` }}>{alive ? '🙂' : '😵'}</div>
        {obstacles.map(obs => <div key={obs.id} style={{ position: 'absolute', bottom: 24, left: `${obs.x}%`, fontSize: 24 }}>{obs.emoji}</div>)}
        {!started && <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,.4)', borderRadius: 14 }}><p style={{ color: '#FBBF24', fontWeight: 800, fontSize: 16 }}>Appuie / Espace pour sauter!</p></div>}
      </div>
      <p style={{ color: 'rgba(255,255,255,.4)', fontSize: 12, marginTop: 8 }}>{palName} doit éviter les obstacles du {theme.world}!</p>
    </div>
  )
}
...
function TapGame({ creature, palette, palName, onComplete }: { creature: string; palette: any; palName: string; onComplete: (score: number) => void }) {
  ...
  const [started, setStarted]   = useState(false)
  const doneRef = useRef(false); const scoreRef = useRef(0)
  useEffect(() => {
    if (!started) return
    const timer = setInterval(() => { setTimeLeft(t => { if (t <= 1) { clearInterval(timer); if (!doneRef.current) { doneRef.current = true; setTimeout(() => onComplete(scoreRef.current), 500) } return 0 } return t - 1 }) }, 1000)
    return () => clearInterval(timer)
  }, [started, onComplete])
  useEffect(() => {
    if (!started || timeLeft === 0) return
    const spawn = setInterval(() => { setTargets(prev => [...prev.slice(-6), { id: Date.now(), x: 5 + Math.random() * 80, y: 10 + Math.random() * 70, emoji: theme.obstacles[Math.floor(Math.random() * theme.obstacles.length)] }]) }, 800)
    return () => clearInterval(spawn)
  }, [started, timeLeft, theme])
  const hit = (id: number) => { setTargets(prev => prev.filter(t => t.id !== id)); scoreRef.current += 10; setScore(s => s + 10) }
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <div onClick={() => !started && setStarted(true)} style={{ width: '100%', maxWidth: 360, height: 200, margin: '0 auto', background: theme.bg, borderRadius: 16, position: 'relative', overflow: 'hidden', cursor: started ? 'default' : 'pointer', border: `2px solid ${palette.main}` }}>
        {!started ? <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><p style={{ color: '#FBBF24', fontWeight: 800, fontSize: 16 }}>Appuie pour commencer!</p></div>
        : timeLeft === 0 ? <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 8 }}><p style={{ color: '#FBBF24', fontWeight: 800, fontSize: 20 }}>Terminé! 🎉</p><p style={{ color: '#fff', fontSize: 16 }}>Score: {score}</p></div>
        : targets.map(tgt => <div key={tgt.id} onClick={() => hit(tgt.id)} style={{ position: 'absolute', left: `${tgt.x}%`, top: `${tgt.y}%`, fontSize: 28, cursor: 'pointer', userSelect: 'none', transform: 'translate(-50%, -50%)' }}>{tgt.emoji}</div>)}
      </div>
      <p style={{ color: 'rgba(255,255,255,.4)', fontSize: 12, marginTop: 8 }}>Attrape les créatures du {theme.world} de {palName}!</p>
    </div>
  )
}
...
function BreatheGame({ palette, palName, onComplete }: { creature: string; palette: any; palName: string; onComplete: (score: number) => void }) {
      ...
      setCount(c => {
        if (c <= 1) {
          setPhase(p => {
            ...
            setCycles(cy => { if (cy + 1 >= TOTAL) { clearInterval(timer); setTimeout(() => onComplete(100), 500) } return cy + 1 })
  ...
  return (
    <div style={{ textAlign: 'center', padding: '8px 0' }}>
      <p style={{ color: 'rgba(255,255,255,.6)', fontSize: 13, marginBottom: 20 }}>{palName} t'accompagne 🌬️ · {TOTAL - cycles} cycles restants</p>
      <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 16 }}>
        <div style={{ width: size, height: size, borderRadius: '50%', background: `radial-gradient(circle, ${color}44, ${color}11)`, border: `3px solid ${color}`, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 1s ease', boxShadow: `0 0 ${size/2}px ${color}44` }}>
          <div style={{ textAlign: 'center' }}>
...
function BreakOverlay({ creature, palette, palName, lang, t, onFinish }: { creature: string; palette: any; palName: string; lang: 'fr'|'cr'; t: typeof T['fr']; onFinish: () => void }) {
  ...
  useEffect(() => {
    const timer = setInterval(() => { setBreakLeft(t => { if (t <= 1) { clearInterval(timer); return 0 } return t - 1 }) }, 1000)
    return () => clearInterval(timer)
  }, [])
  const breakMins = String(Math.floor(breakLeft / 60)).padStart(2, '0')
  const breakSecs = String(breakLeft % 60).padStart(2, '0')
  const gameNames: Record<GameType, string> = { memory: '🃏 Carte mémoire', dodge: '🚀 Évite les obstacles', tap: '🎯 Clique vite', breathe: '🌬️ Respiration guidée' }
  const handleGameComplete = (score: number) => { setGameScore(score); setGameDone(true) }
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 200, background: `linear-gradient(160deg, #0B1F4B 0%, ${palette.main}33 100%)`, display: 'flex', flexDirection: 'column', fontFamily: 'var(--font-jakarta)', animation: 'slideUp .4s ease' }}>
      <div style={{ padding: '20px 20px 16px', textAlign: 'center', borderBottom: '1px solid rgba(255,255,255,.08)', flexShrink: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <div style={{ background: 'rgba(34,197,94,.2)', border: '1px solid #22C55E', borderRadius: 99, padding: '4px 14px' }}><span style={{ color: '#22C55E', fontWeight: 800, fontSize: 13 }}>🎉 +50 pts gagnés!</span></div>
          <div style={{ background: 'rgba(251,191,36,.15)', border: '1px solid #FBBF24', borderRadius: 99, padding: '4px 14px' }}><span style={{ color: '#FBBF24', fontWeight: 800, fontSize: 13 }}>⏱ {breakMins}:{breakSecs}</span></div>
        </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 16px' }}>
        {!gameDone ? (
          ...
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,.6)', fontSize: 15, marginBottom: 4 }}>{t.breakScore}: <strong style={{ color: '#fff' }}>{gameScore}</strong></p>
            <p style={{ color: 'rgba(255,255,255,.4)', fontSize: 13 }}>{palName} est fier de toi! 🌟</p>
          </div>
        )}
      </div>
      <div style={{ padding: '12px 16px 24px', flexShrink: 0 }}>
        <button onClick={onFinish} disabled={!gameDone && breakLeft > 0} style={{ width: '100%', padding: '14px', background: gameDone || breakLeft === 0 ? `linear-gradient(135deg, #0B1F4B, ${palette.main})` : 'rgba(255,255,255,.08)', color: gameDone || breakLeft === 0 ? '#FBBF24' : 'rgba(255,255,255,.3)', border: 'none', borderRadius: 16, fontWeight: 800, fontSize: 15, cursor: gameDone || breakLeft === 0 ? 'pointer' : 'default', fontFamily: 'var(--font-jakarta)', transition: 'all .2s' }}>
...
export default function AskPage() {
  ...
  const distractionIdRef = useRef<string | null>(null)
  ...
  useEffect(() => {
    ...
    const handleVisibility = async () => {
      if (document.visibilityState === 'hidden') {
        const now = new Date(); leftAtRef.current = now
        if (child) {
          const { data } = await supabase.from('distractions').insert({ child_id: child.id, left_at: now.toISOString(), timer_reset: false }).select('id').single()
          if (data) distractionIdRef.current = data.id
        }
        setGraceLeft(GRACE_PERIOD); setShowGrace(true)
        graceIntervalRef.current = setInterval(() => {
          setGraceLeft(prev => {
            if (prev <= 1) {
              clearInterval(graceIntervalRef.current); setShowGrace(false)
              setSessionStart(new Date()); setElapsed(0); setBreakShown(false)
              if (distractionIdRef.current && child) {
                const returnedAt = new Date()
                const durationAway = leftAtRef.current ? Math.floor((returnedAt.getTime() - leftAtRef.current.getTime()) / 1000) : GRACE_PERIOD
                supabase.from('distractions').update({ returned_at: returnedAt.toISOString(), duration_away_s: durationAway, timer_reset: true }).eq('id', distractionIdRef.current).then(() => { distractionIdRef.current = null })
              }
              setMessages(prev => [...prev, { role: 'assistant', content: lang === 'fr' ? `⏱️ Tu as quitté l'application pendant plus de 3 minutes. Le timer a été remis à zéro — mais ne t'inquiète pas, ta conversation est toujours là. Reprends là où tu en étais! 💪` : `⏱️ Ou te kite aplikasyon an pandan plis pase 3 minit. Timer a reyalize — men pa enkyete ou, konvèsasyon an la toujou. Kontinye kote ou te ye a! 💪` }])
              return 0
            }
            return prev - 1
          })
      ...
      if (document.visibilityState === 'visible' && showGrace) {
        ...
        if (distractionIdRef.current && leftAtRef.current && child) {
          ...
          supabase.from('distractions').update({ returned_at: returnedAt.toISOString(), duration_away_s: durationAway, timer_reset: false }).eq('id', distractionIdRef.current).then(() => { distractionIdRef.current = null })
        }
      }
    }
    document.addEventListener('visibilitychange', handleVisibility)
    return () => { document.removeEventListener('visibilitychange', handleVisibility); clearInterval(graceIntervalRef.current) }
  }, [phase, sessionDone, child, lang, showGrace])


  useEffect(() => { return () => clearInterval(graceIntervalRef.current) }, [])
  ...
  const handleBreakFinish = () => {
    setShowBreak(false); setPomodorosCompleted(p => p + 1); setSessionStart(new Date()); setElapsed(0); setBreakShown(false)
    setMessages(m => [...m, { role: 'assistant', content: lang === 'fr' ? `Excellent! Pause terminée 💪 Nouveau Pomodoro lancé — 25 minutes de focus sur **${getSubjectLabel(subject)}**${topic ? ` · ${topic}` : ''}. Continue comme ça!` : `Ekselan! Repo fini 💪 Nouvo Pomodoro kòmanse — 25 minit fokis sou **${getSubjectLabel(subject)}**. Kontinye konsa!` }])
  }
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ messages: newMessages.map(m => ({ role: m.role, content: m.content })), palName, personality, lang, subject, grade, chapter: topic ?? null, pomodoro: pomodoroOn, palPalette, creature, childId: child.id }),
      })
      ...
      if (data.message) {
        ...
        setMessages(m => [...m, { role: 'assistant', content: data.message, imageUrl, shortAnswer: data.shortAnswer || '', imagePrompt: data.imagePrompt || '', savedAsFlashcard: false }])
      }
  ...
  const saveFlashcard = async (msgIndex: number) => {
    ...
    await supabase.from('flashcards').insert({ child_id: child.id, subject: getSubjectLabel(subject), question: userMsg?.content || getSubjectLabel(subject), answer: aiMsg.shortAnswer || aiMsg.content.slice(0, 150), image_prompt: aiMsg.imagePrompt || '' })
  ...
  const endSession = async () => {
    ...
    await supabase.from('study_sessions').insert({ child_id: child.id, subject: getSubjectLabel(subject), technique: pomodoroOn ? 'pomodoro' : 'free', duration_mins: durationMins, points_earned: pts })
    if (pts > 0) {
    ...
    setPtsEarned(pts); setSessionDone(true); setSessionStart(null)
    setShowGrace(false); clearInterval(graceIntervalRef.current)
  }
    ...
    <div style={{ minHeight: '100%', background: '#F4F7FF', fontFamily: 'var(--font-jakarta)' }}>
      <div style={{ background: 'linear-gradient(160deg, #0B1F4B, #13306B)', padding: isWide ? '32px 32px 36px' : '24px 20px 36px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div>
            <h1 style={{ fontFamily: 'var(--font-fredoka)', color: '#fff', fontSize: isWide ? 32 : 26, fontWeight: 700 }}>{t.title} {palName}</h1>
      ...
      <div style={{ padding: isWide ? '28px 32px' : '20px 18px', maxWidth: 640, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {ALL_SUBJECTS.map(s => {
              const cfg = SUBJECT_CONFIG[s]; const isActive = subject === s
              return (
                <button key={s} onClick={() => setSubject(s)} style={{ padding: '14px 16px', borderRadius: 16, background: isActive ? '#0B1F4B' : '#fff', color: isActive ? '#FBBF24' : '#64748B', fontWeight: 700, fontSize: 13, cursor: 'pointer', border: `1.5px solid ${isActive ? '#0B1F4B' : '#E2E8F0'}`, fontFamily: 'var(--font-jakarta)', transition: 'all .15s', display: 'flex', alignItems: 'center', gap: 10, textAlign: 'left' }}>
        ...
        {subtopics.length > 0 && (
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              <button onClick={() => setTopic(null)} style={{ padding: '7px 14px', borderRadius: 99, background: topic === null ? '#0B1F4B' : '#fff', color: topic === null ? '#FBBF24' : '#64748B', fontWeight: 700, fontSize: 12, cursor: 'pointer', border: `1.5px solid ${topic === null ? '#0B1F4B' : '#E2E8F0'}`, fontFamily: 'var(--font-jakarta)', transition: 'all .15s' }}>{t.topicGeneral}</button>
              {subtopics.map(tp => (
                <button key={tp} onClick={() => setTopic(topic === tp ? null : tp)} style={{ padding: '7px 14px', borderRadius: 99, background: topic === tp ? palette.main : '#fff', color: topic === tp ? '#fff' : '#64748B', fontWeight: 600, fontSize: 12, cursor: 'pointer', border: `1.5px solid ${topic === tp ? palette.main : '#E2E8F0'}`, fontFamily: 'var(--font-jakarta)', transition: 'all .15s' }}>{tp}</button>
              ))}
            </div>
      ...
      {showGrace && (
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 300, background: 'linear-gradient(135deg, #EF4444, #DC2626)', padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', animation: 'slideDown .3s ease' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <p style={{ color: '#fff', fontWeight: 800, fontSize: 13, margin: 0 }}>Tu as quitté l'application!</p>
              <p style={{ color: 'rgba(255,255,255,.8)', fontSize: 11, margin: 0 }}>Reviens maintenant — le timer repart dans {graceMins}:{graceSecs}</p>
            </div>
          </div>
          <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'rgba(255,255,255,.15)', border: '2px solid rgba(255,255,255,.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <p style={{ color: '#fff', fontWeight: 800, fontSize: 14, fontFamily: 'var(--font-fredoka)', margin: 0 }}>{graceLeft}</p>
          </div>
        </div>
      )}
      ...
      {sessionDone && (
        <div style={{ background: ptsEarned > 0 ? 'linear-gradient(135deg, #D1FAE5, #A7F3D0)' : '#FEF3C7', padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          <p style={{ fontWeight: 700, color: ptsEarned > 0 ? '#065F46' : '#92400E', fontSize: 14 }}>
            {ptsEarned > 0 ? `${t.sessionSaved}${ptsEarned} ${t.points} · ${pomodorosCompleted} Pomodoro${pomodorosCompleted > 1 ? 's' : ''}` : '⏱️ Complète un Pomodoro pour gagner des points!'}
          </p>
          <button onClick={() => { setPhase('setup'); setMessages([]); setSessionDone(false); setSessionLimitReached(false) }} style={{ background: ptsEarned > 0 ? '#065F46' : '#92400E', color: '#fff', border: 'none', borderRadius: 10, padding: '7px 16px', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-jakarta)' }}>
            Nouvelle session
          </button>
        </div>
      )}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        {messages.map((msg, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start', gap: 8, alignItems: 'flex-end' }}>
            {msg.role === 'assistant' && <div style={{ flexShrink: 0, marginBottom: 2 }}><PalSVG creature={creature} shape={child.pal?.bodyShape || 'round'} palette={palette} feature={child.pal?.feature || 'eyes'} size={28} /></div>}
            {msg.role === 'assistant' ? (
              <div style={{ maxWidth: '82%', display: 'flex', flexDirection: 'column', gap: 6 }}>
                {msg.imageUrl && <div style={{ borderRadius: '16px 16px 4px 16px', overflow: 'hidden', border: `2px solid ${palette.main}33` }}><img src={msg.imageUrl} alt="" style={{ width: '100%', maxHeight: 180, objectFit: 'cover', display: 'block' }} onError={e => { (e.target as HTMLImageElement).parentElement!.style.display = 'none' }} /></div>}
                ...
                {i > 0 && msg.shortAnswer && (
                  <button onClick={() => saveFlashcard(i)} disabled={msg.savedAsFlashcard || savedIds.has(i)} style={{ alignSelf: 'flex-start', background: msg.savedAsFlashcard || savedIds.has(i) ? 'rgba(34,197,94,.2)' : 'rgba(255,255,255,.07)', border: `1px solid ${msg.savedAsFlashcard || savedIds.has(i) ? '#22C55E' : 'rgba(255,255,255,.15)'}`, borderRadius: 99, padding: '4px 12px', color: msg.savedAsFlashcard || savedIds.has(i) ? '#22C55E' : 'rgba(255,255,255,.45)', fontSize: 11, fontWeight: 700, cursor: msg.savedAsFlashcard || savedIds.has(i) ? 'default' : 'pointer', fontFamily: 'var(--font-jakarta)', transition: 'all .2s' }}>
                    {msg.savedAsFlashcard || savedIds.has(i) ? t.flashcardSaved : `🗂️ ${t.saveFlashcard}`}
                  </button>
                )}
              </div>
            ) : (
              <div style={{ maxWidth: '78%', padding: '11px 15px', borderRadius: '16px 16px 4px 16px', background: '#FBBF24', color: '#0B1F4B', fontSize: 14, lineHeight: 1.6, fontWeight: 600 }}>{msg.content}</div>
            )}
          </div>
        ))}
        {loading && (
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8 }}>
            <div style={{ flexShrink: 0 }}><PalSVG creature={creature} shape={child.pal?.bodyShape || 'round'} palette={palette} feature={child.pal?.feature || 'eyes'} size={28} /></div>
            <div style={{ background: 'rgba(59,82,212,.3)', border: '1px solid rgba(59,82,212,.3)', borderRadius: '16px 16px 16px 4px', padding: '12px 16px', display: 'flex', gap: 5 }}>
              {[0,1,2].map(n => <div key={n} style={{ width: 7, height: 7, borderRadius: '50%', background: 'rgba(255,255,255,.4)', animation: `typing 1.2s ${n * .2}s infinite` }} />)}
            </div>
          </div>
        )}
        <div ref={chatEndRef} />
      ...
      <div style={{ padding: '10px 14px 20px', background: '#0B1F4B', borderTop: '1px solid rgba(255,255,255,.06)', flexShrink: 0 }}>
        {sessionLimitReached ? (
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 10 }}>
            <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendMessage()} placeholder={t.typeMessage}
