...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    supabase.from('mentors').select('id, name, field, avatar_emoji').limit(1).maybeSingle()
      .then(async ({ data }: any) => {
        if (!data) return
        const { count } = await supabase.from('mentor_episodes').select('id', { count: 'exact' }).eq('mentor_id', data.id).eq('published', true)
        setNextMentor({ ...data, episode_count: count || 0 })
      })


    supabase.from('news_articles').select('id, title, subject, hero, hero_name').eq('published', true).limit(3)
      .then(({ data }: any) => { if (data) setArticles(data) })


    // Sessions today + tour check
    supabase.from('children').select('sessions_today, tour_completed').eq('id', child.id).single()
      .then(({ data }: any) => {
        if (data) {
    ...
    const computeStreak = async () => {
      const { data: schedule } = await supabase.from('focus_schedules').select('days, start_time, end_time').eq('child_id', child.id).eq('active', true).maybeSingle()
      const { data: pomodoros } = await supabase.from('study_sessions').select('created_at').eq('child_id', child.id).eq('technique', 'pomodoro').gt('points_earned', 0).order('created_at', { ascending: false })
      if (!pomodoros || pomodoros.length === 0) { setStreak(0); return }
      if (!schedule || !schedule.days || schedule.days.length === 0) {
        setStreak(new Set(pomodoros.map((s: any) => new Date(s.created_at).toDateString())).size)
        return
      }
      ...
      for (const s of pomodoros) {
        const d = new Date(s.created_at); const h = d.getHours(); const m = d.getMinutes()
        if ((h > startH || (h === startH && m >= startM)) && (h < endH || (h === endH && m <= endM)))
          completedDates.add(d.toDateString())
      }
      let count = 0; const today = new Date(); today.setHours(0,0,0,0); const cursor = new Date(today)
      for (let i = 0; i < 90; i++) {
        const dayName = DAY_NAMES[cursor.getDay()]
        if (scheduledDays.includes(dayName)) {
          const hit = completedDates.has(cursor.toDateString())
          if (!hit && cursor.toDateString() !== today.toDateString()) break
          if (hit) count++
        }
  ...
  if (loading) return <div style={{ minHeight:'100vh', background:'#0B1F4B' }} />
  ...
  const UpgradeBanner = () => !isFreeLimit ? null : (
    <div style={{ background:'linear-gradient(135deg, #0B1F4B, #1a3266)', border:'1px solid rgba(251,191,36,0.3)', borderRadius: isWide ? 20 : 0, padding:'16px 20px', display:'flex', alignItems:'center', gap:14 }}>
      <div style={{ flex:1 }}>
        <p style={{ color:'#FBBF24', fontWeight:800, fontSize:14, margin:'0 0 4px', fontFamily:'var(--font-fredoka)' }}>Tu as utilisé ta session gratuite d'aujourd'hui! 🌙</p>
        <p style={{ color:'rgba(255,255,255,0.55)', fontSize:12, margin:0, lineHeight:1.5 }}>C'est déjà une belle journée d'étude. Reviens demain, ou demande à tes parents d'activer les sessions illimitées.</p>
      </div>
      <button onClick={() => router.push('/pricing')} style={{ background:'#FBBF24', border:'none', borderRadius:10, padding:'8px 14px', color:'#0B1F4B', fontSize:11, fontWeight:800, cursor:'pointer', flexShrink:0, fontFamily:'var(--font-jakarta)', whiteSpace:'nowrap' }}>
        Voir les plans →
      </button>
    </div>
  )
  ...
  const LeftCol = () => (
    <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
      <div style={{ background:'linear-gradient(160deg, #0B1F4B 0%, #13306B 100%)', borderRadius: isWide ? 24 : 0, padding: isWide ? '28px 28px 0' : '20px 20px 0', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:20, position:'relative', zIndex:2 }}>
          <div>
            ...
            <h1 style={{ fontFamily:'var(--font-fredoka)', color:'#fff', fontSize: isWide ? 34 : 28, fontWeight:700, lineHeight:1.1 }}>{child.name}</h1>
          </div>
          <div style={{ position:'relative' }}>
            <div style={{ width:46, height:46, borderRadius:14, background:'rgba(255,255,255,.08)', border:'1px solid rgba(255,255,255,.1)', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', fontSize:20 }}>🔔</div>
        ...
        {/* Stats — tour target */}
        <div id="tour-stats" style={{ display:'flex', gap:10, marginBottom:24, position:'relative', zIndex:2 }}>
          {[
        ...
        {/* Pal greeting — tour target */}
        <div id="tour-pal" style={{ background:'rgba(255,255,255,.06)', border:'1px solid rgba(255,255,255,.1)', borderRadius:'20px 20px 0 0', padding:'18px 18px 28px', display:'flex', alignItems:'center', gap:16, position:'relative', zIndex:2, opacity: animIn ? 1 : 0, transform: animIn ? 'translateY(0)' : 'translateY(12px)', transition:'all .4s ease' }}>
          <div style={{ animation:'float 3s ease-in-out infinite', flexShrink:0 }}>
            <PalSVG creature={child.pal?.creature || 'land'} shape={child.pal?.bodyShape || 'round'} palette={palette} feature={child.pal?.feature || 'eyes'} size={isWide ? 88 : 72} />
          </div>
          <div style={{ flex:1 }}>
            <p style={{ color:'#FBBF24', fontWeight:800, fontSize:13, marginBottom:5 }}>{palName} {t.says}:</p>
            <p style={{ color:'rgba(255,255,255,.85)', fontSize:15, lineHeight:1.6 }}>"{greeting}"</p>
          </div>
        </div>
      </div>
  ...
  const RightCol = () => (
    <div style={{ display:'flex', flexDirection:'column', gap:20, padding: isWide ? '0' : '0 18px' }}>
      ...
      {/* Articles — tour target */}
      <div id="tour-articles">
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
          <h2 style={{ fontFamily:'var(--font-fredoka)', color:'#0B1F4B', fontSize:20, fontWeight:600 }}>{t.dernieres}</h2>
          <button onClick={() => router.push('/home/quests')} style={{ color:palette.main, fontSize:13, fontWeight:700, background:'none', border:'none', cursor:'pointer' }}>{t.voirTout}</button>
        </div>
        {articles.length > 0 ? (
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {articles.map((a: any) => (
              <div key={a.id} onClick={() => router.push('/home/quests')} style={{ background:'#fff', borderRadius:18, padding:'14px 16px', border:'1.5px solid #E2E8F0', display:'flex', gap:12, alignItems:'center', cursor:'pointer', transition:'transform .15s, box-shadow .15s' }}
      ...
      {/* Mentors — tour target */}
      <div id="tour-mentors">
        <h2 style={{ fontFamily:'var(--font-fredoka)', color:'#0B1F4B', fontSize:20, fontWeight:600, marginBottom:12 }}>{t.prochainMentor}</h2>
        {nextMentor ? (
          <div style={{ background:'linear-gradient(135deg, #EFF6FF, #F0F9FF)', borderRadius:20, padding:'18px', border:'1.5px solid #BFDBFE', display:'flex', alignItems:'center', gap:14 }}>
            <div style={{ width:54, height:54, borderRadius:16, background:'#DBEAFE', display:'flex', alignItems:'center', justifyContent:'center', fontSize:26, flexShrink:0 }}>{nextMentor.avatar_emoji || '🎬'}</div>
            <div style={{ flex:1 }}>
              ...
              <p style={{ color:'#64748B', fontSize:12, marginTop:3 }}>{nextMentor.episode_count || 0} épisode{(nextMentor.episode_count||0)>1?'s':''} disponible{(nextMentor.episode_count||0)>1?'s':''}</p>
            </div>
            <div onClick={() => router.push('/home/mentors')} style={{ background:'#3B52D4', color:'#fff', borderRadius:12, padding:'10px 16px', fontSize:13, fontWeight:700, cursor:'pointer', flexShrink:0 }}>▶ Regarder</div>
          </div>
        ) : (
          <div style={{ background:'#fff', borderRadius:20, padding:28, border:'1.5px dashed #E2E8F0', textAlign:'center' }}>
            <p style={{ fontSize:36, marginBottom:10 }}>🎬</p>
    ...
    <div style={{ minHeight:'100%', background:'#F4F7FF' }}>
      {isWide ? (
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:24, padding:28, maxWidth:1200, margin:'0 auto' }}>
          {isFreeLimit && <div style={{ gridColumn:'1/-1' }}><UpgradeBanner /></div>}
          <LeftCol />
          <RightCol />
        </div>
      ) : (
