...
const CATEGORIES = [
  { id:'all', label:'Tout' }, { id:'toy', label:'Jouets' },
  { id:'game', label:'Jeux' }, { id:'merch', label:'Merch' }, { id:'book', label:'Livres' },
]
const SORTS = [
  { id:'default', label:'Recommandés' }, { id:'price_asc', label:'Prix croissant' },
  { id:'price_desc', label:'Prix décroissant' }, { id:'new', label:'Nouveautés' },
]
      ...
      <div style={{ position:'fixed', top:0, right:0, bottom:0, width:360, background:C.white, zIndex:50, display:'flex', flexDirection:'column', boxShadow:'-4px 0 40px rgba(0,0,0,0.15)', fontFamily:'var(--font-jakarta)' }}>
          ...
          {cart.length === 0 ? (
            ...
            <div key={item.product.id} style={{ display:'flex', gap:12, marginBottom:16, paddingBottom:16, borderBottom:`1px solid ${C.border}` }}>
              <div style={{ width:64, height:64, borderRadius:10, background:C.bg, display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden', flexShrink:0 }}>
                {item.product.image_url ? <img src={item.product.image_url} alt={item.product.name} style={{ width:'100%', height:'100%', objectFit:'cover' }} /> : <span style={{ fontSize:28 }}>🎁</span>}
              </div>
        ...
        {cart.length > 0 && (
          <div style={{ padding:'16px 20px', borderTop:`1px solid ${C.border}` }}>
            ...
            <button onClick={onCheckout} disabled={loading} style={{ width:'100%', background:loading?C.faint:C.navy, border:'none', borderRadius:12, padding:'14px 0', color:C.gold, fontSize:14, fontWeight:800, cursor:loading?'not-allowed':'pointer', fontFamily:'var(--font-jakarta)' }}>
              {loading ? 'Redirection...' : 'Passer la commande →'}
            </button>
            <p style={{ color:C.faint, fontSize:10, textAlign:'center', margin:'10px 0 0' }}>Paiement sécurisé par Stripe · Livraison au Canada</p>
          </div>
        )}
      </div>
    </>
  ...
  return (
    <div style={{ background:C.white, border:`1px solid ${inCart?C.navy:C.border}`, borderRadius:16, overflow:'hidden', transition:'border-color 0.2s', display:'flex', flexDirection:'column' }}>
      <div style={{ aspectRatio:'1/1', background:C.bg, display:'flex', alignItems:'center', justifyContent:'center', position:'relative', overflow:'hidden' }}>
        {product.image_url ? <img src={product.image_url} alt={product.name} style={{ width:'100%', height:'100%', objectFit:'cover' }} /> : <span style={{ fontSize:52 }}>🎁</span>}
        <div style={{ position:'absolute', top:10, left:10, display:'flex', flexDirection:'column', gap:4 }}>
          {product.is_new && <span style={{ background:C.navy, color:C.gold, fontSize:9, fontWeight:700, borderRadius:6, padding:'3px 8px', fontFamily:'var(--font-jakarta)' }}>NOUVEAU</span>}
          {isWished && <span style={{ background:C.goldDim, border:`1px solid ${C.goldBorder}`, color:C.navy, fontSize:9, fontWeight:700, borderRadius:6, padding:'3px 8px', fontFamily:'var(--font-jakarta)' }}>❤️ Souhaité</span>}
        </div>
        {product.stock > 0 && product.stock < 4 && <span style={{ position:'absolute', top:10, right:10, background:C.red, color:C.white, fontSize:9, fontWeight:700, borderRadius:6, padding:'3px 8px' }}>Plus que {product.stock}!</span>}
      </div>
      <div style={{ padding:'12px 14px 14px', flex:1, display:'flex', flexDirection:'column', fontFamily:'var(--font-jakarta)' }}>
        ...
        {discountPct > 0 && <p style={{ color:C.green, fontSize:10, fontWeight:600, margin:'0 0 12px' }}>Économie de {saved(product.price_cad, discountPct).toFixed(2)} CAD</p>}
        <button onClick={() => onAdd(product)} style={{ width:'100%', background:inCart?C.goldDim:C.navy, border:`1px solid ${inCart?C.goldBorder:C.navy}`, borderRadius:10, padding:'10px 0', color:inCart?C.navy:C.gold, fontSize:12, fontWeight:700, cursor:'pointer', transition:'all 0.2s', fontFamily:'var(--font-jakarta)' }}>
...
export default function ParentPage() {
  ...
  async function loadAll() {
    ...
    const { data: profile } = await supabase.from('profiles').select('full_name, plan').eq('id', u.id).single()
  ...
  async function handleCheckout() {
    ...
    const res = await fetch('/api/checkout', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ cartItems, childId: child.id }) })
    const { url } = await res.json()
    if (url) { window.location.href = url } else { showToast('Erreur lors du paiement. Réessayez.'); setCheckLoading(false) }
  }
  ...
  async function changePassword() {
    ...
    setPwForm({ current:'', next:'', confirm:'' }); showToast('Mot de passe modifié.')
  }
      ...
      <div style={{ background:C.navy, padding:'0 20px', position:'sticky', top:0, zIndex:30 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', height:60 }}>
          <div>
            ...
            <h1 style={{ color:C.white, fontSize:18, fontWeight:800, margin:0, fontFamily:'var(--font-fredoka)' }}>Bonjour, {parentName}</h1>
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <button onClick={() => setShowCart(true)} style={{ background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.15)', borderRadius:10, padding:'8px 14px', cursor:'pointer', display:'flex', alignItems:'center', gap:8 }}>
              <span style={{ fontSize:16 }}>🛒</span>
              <span style={{ color:C.white, fontWeight:700, fontSize:13 }}>Panier</span>
              {cartCount > 0 && <div style={{ background:C.gold, borderRadius:'50%', width:20, height:20, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}><span style={{ color:C.navy, fontSize:11, fontWeight:800 }}>{cartCount}</span></div>}
            </button>
          </div>
        </div>
        ...
        <div style={{ padding:'22px 20px' }}>
          {wishItems.filter(p => !inCart(p.id)).length > 0 && (
            <div style={{ background:C.goldDim, border:`1px solid ${C.goldBorder}`, borderRadius:16, padding:'14px 16px', marginBottom:22 }}>
              <p style={{ color:C.navy, fontWeight:800, fontSize:12, margin:'0 0 2px', textTransform:'uppercase', letterSpacing:'0.5px' }}>Liste de souhaits de {child?.name}</p>
              <p style={{ color:C.muted, fontSize:11, margin:'0 0 12px' }}>Ces articles ont été ajoutés par {child?.name}. Ajoutez-les au panier directement.</p>
              <div style={{ display:'flex', gap:8, overflowX:'auto', paddingBottom:4 }}>
                {wishItems.filter(p => !inCart(p.id)).map(p => (
                  <div key={p.id} style={{ flexShrink:0, background:C.white, border:`1px solid ${C.border}`, borderRadius:12, padding:'10px 12px', display:'flex', gap:10, alignItems:'center', minWidth:220 }}>
                    ...
                    <button onClick={() => addToCart(p)} style={{ background:C.navy, border:'none', borderRadius:8, padding:'6px 12px', color:C.gold, fontSize:11, fontWeight:700, cursor:'pointer', flexShrink:0 }}>+ Panier</button>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ display:'flex', gap:10, marginBottom:14, flexWrap:'wrap' }}>
            <div style={{ flex:1, minWidth:180, position:'relative' }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher un jouet..." style={{ width:'100%', background:C.white, border:`1px solid ${C.border}`, borderRadius:10, padding:'10px 12px 10px 34px', color:C.text, fontSize:13, fontFamily:'var(--font-jakarta)', outline:'none', boxSizing:'border-box' as const }} />
            </div>
            <select value={sort} onChange={e => setSort(e.target.value)} style={{ background:C.white, border:`1px solid ${C.border}`, borderRadius:10, padding:'10px 14px', color:C.navy, fontSize:13, fontFamily:'var(--font-jakarta)', cursor:'pointer', outline:'none', fontWeight:600 }}>
          ...
          <div style={{ display:'flex', gap:10, marginBottom:16, flexWrap:'wrap', alignItems:'center' }}>
            ...
            <button onClick={() => setWishOnly(!wishOnly)} style={{ background:wishOnly?C.navy:C.white, border:`1px solid ${wishOnly?C.navy:C.border}`, borderRadius:10, padding:'9px 16px', color:wishOnly?C.gold:C.muted, fontSize:12, fontWeight:700, cursor:'pointer', transition:'all 0.2s', whiteSpace:'nowrap' }}>
              ❤️ Souhaités seulement
            </button>
          </div>
          <div style={{ display:'flex', gap:8, marginBottom:20, alignItems:'center', flexWrap:'wrap' }}>
            {CATEGORIES.map(cat => (
              <button key={cat.id} onClick={() => setCategory(cat.id)} style={{ background:category===cat.id?C.navy:C.white, border:`1px solid ${category===cat.id?C.navy:C.border}`, borderRadius:99, padding:'7px 18px', color:category===cat.id?C.gold:C.muted, fontSize:12, fontWeight:700, cursor:'pointer', transition:'all 0.2s' }}>
                {cat.label}
              </button>
            ))}
            <span style={{ marginLeft:'auto', color:C.faint, fontSize:12 }}>{filtered.length} résultat{filtered.length!==1?'s':''}</span>
          </div>
          {filtered.length === 0 ? (
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(200px, 1fr))', gap:16 }}>
              {filtered.map(p => <ProductCard key={p.id} product={p} discountPct={discountPct} inCart={inCart(p.id)} isWished={inWL(p.id)} onAdd={addToCart} />)}
            </div>
          )}
        </div>
      )}
      ...
      {tab === 'wishlist' && (
        <div style={{ padding:'22px 20px' }}>
          <Card style={{ marginBottom:20 }}>
            ...
            <p style={{ color:C.muted, fontSize:11, margin:0 }}>{points} points accumulés{discountPct < 30 && ` · Prochain palier à ${TIERS.find(t => t > points)} points`}</p>
          </Card>
          ...
          <p style={{ color:C.faint, fontSize:11, margin:'0 0 14px' }}>{child?.name} a ajouté {wishlist.length} article(s). Le rabais de {discountPct}% est appliqué automatiquement.</p>
          {wishItems.length === 0 ? (
            <Card style={{ textAlign:'center', padding:'28px 20px' }}>
              <p style={{ color:C.faint, fontSize:13, margin:0 }}>{child?.name} n'a pas encore ajouté de jouets.</p>
            </Card>
          ) : wishItems.map(p => (
            <div key={p.id} style={{ background:C.white, border:`1px solid ${C.border}`, borderRadius:14, padding:'14px', display:'flex', gap:12, alignItems:'center', marginBottom:10 }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <p style={{ color:C.green, fontSize:10, fontWeight:600, margin:0 }}>Économie de {saved(p.price_cad, discountPct).toFixed(2)} CAD grâce aux points de {child?.name}</p>
              </div>
      ...
      {tab === 'profile' && (
        <div style={{ padding:'22px 20px' }}>
            ...
            <div style={{ marginBottom:16 }}>
              {[
                ...
                <div key={i} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
                  ...
                  <span style={{ color:f.included?C.text:C.faint, fontSize:13, textDecoration:f.included?'none':'line-through' }}>{f.label}</span>
                </div>
              ))}
            </div>
          ...
          <Card style={{ marginBottom:22 }}>
            ...
            <button onClick={() => showToast('Informations mises à jour.')} style={{ width:'100%', background:C.navy, border:'none', borderRadius:10, padding:'11px 0', color:C.gold, fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'var(--font-jakarta)' }}>Sauvegarder</button>
          </Card>
          ...
          <Card style={{ marginBottom:22 }}>
            ...
            <button onClick={changePassword} style={{ width:'100%', background:C.navy, border:'none', borderRadius:10, padding:'11px 0', color:C.gold, fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'var(--font-jakarta)' }}>Modifier le mot de passe</button>
          </Card>
          ...
          <Card style={{ marginBottom:22 }}>
            ...
            <button onClick={saveSchedule} style={{ width:'100%', background:C.navy, border:'none', borderRadius:10, padding:'11px 0', color:C.gold, fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'var(--font-jakarta)' }}>Sauvegarder l'horaire</button>
          </Card>
          ...
          <Card>
            <button onClick={signOut} style={{ width:'100%', background:'transparent', border:`1px solid ${C.navy}`, borderRadius:10, padding:'11px 0', color:C.navy, fontSize:12, fontWeight:700, cursor:'pointer', marginBottom:10, fontFamily:'var(--font-jakarta)' }}>Se déconnecter</button>
            <button style={{ width:'100%', background:'transparent', border:`1px solid ${C.red}`, borderRadius:10, padding:'11px 0', color:C.red, fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'var(--font-jakarta)' }}>Supprimer le compte</button>
          </Card>
        </div>
      )}


      {showCart && <CartDrawer cart={cart} discountPct={discountPct} onClose={() => setShowCart(false)} onQty={setQty} onRemove={removeFromCart} onCheckout={handleCheckout} loading={checkLoading} />}


      {cartCount > 0 && !showCart && (
        <button onClick={() => setShowCart(true)} style={{ position:'fixed', bottom:24, right:24, background:C.navy, border:'none', borderRadius:99, padding:'14px 22px', color:C.gold, fontWeight:800, fontSize:13, cursor:'pointer', boxShadow:'0 8px 32px rgba(11,31,75,0.3)', display:'flex', alignItems:'center', gap:10, zIndex:20, fontFamily:'var(--font-jakarta)' }}>
