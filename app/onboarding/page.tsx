...
function PalSVG({ creature, shape, palette, feature, size = 200 }: {
  creature: string; shape: string; palette: typeof PALETTES[0];
  feature: string; size?: number;
}) {
  const bodyShape = BODY_SHAPES.find(b => b.id === shape)?.shape
    || BODY_SHAPES[0].shape
  ...
  return (
    <svg viewBox="0 0 100 100" width={size} height={size} style={{ filter: `drop-shadow(0 8px 24px ${palette.glow})`, overflow: 'visible' }}>
      <defs>
      ...
      {creatureEl()}
      {featureEl()}
      <path d={bodyShape} fill="url(#bodyGrad)" stroke={palette.main} strokeWidth="2" />
      {feature !== 'eyes' && (
        <>
      ...
      <div style={{
        ...
        {step === 0 && (
          <div style={{ textAlign: 'center' }}>
            ...
            <h1 style={{
              fontFamily: 'var(--font-fredoka)', color: '#fff',
              fontSize: 48, fontWeight: 700, marginBottom: 16, letterSpacing: 2,
            }}>
              NOVERE
            </h1>
            <p style={{ color: 'rgba(255,255,255,.55)', fontSize: 16, lineHeight: 1.7, marginBottom: 12, maxWidth: 380, margin: '0 auto 12px' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,.3)', fontSize: 14, marginBottom: 48 }}>
              Chaque explorer a besoin d'un compagnon.
            </p>

            {/* Child name + grade before starting */}
            ...
            <button
              ...
              style={{
                ...btnStyle,
                background: childName.trim() ? '#FBBF24' : 'rgba(255,255,255,.1)',
                color: childName.trim() ? '#0B1F4B' : 'rgba(255,255,255,.3)',
        ...
        {step === 1 && (
          <div style={{ textAlign: 'center' }}>
            <div style={{ marginBottom: 40 }}>
              <p style={{ color: 'rgba(255,255,255,.4)', fontSize: 14, marginBottom: 20 }}>
                Bonjour, {childName} 👋
              </p>
        ...
        {step === 2 && (
          <div>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {CREATURE_TYPES.map(c => (
                  <button key={c.id} onClick={() => setCreature(c.id)} style={{
                    ...choiceBtn,
                    background: creature === c.id ? 'rgba(251,191,36,.15)' : 'rgba(255,255,255,.05)',
                    border: `1.5px solid ${creature === c.id ? '#FBBF24' : 'rgba(255,255,255,.1)'}`,
                  }}>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {BODY_SHAPES.map(b => (
                  <button key={b.id} onClick={() => setBodyShape(b.id)} style={{
                    ...choiceBtn,
                    background: bodyShape === b.id ? 'rgba(251,191,36,.15)' : 'rgba(255,255,255,.05)',
                    border: `1.5px solid ${bodyShape === b.id ? '#FBBF24' : 'rgba(255,255,255,.1)'}`,
                  }}>
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                {PALETTES.map(p => (
                  <button key={p.id} onClick={() => setPaletteId(p.id)} style={{
                    ...
                    background: `linear-gradient(135deg, ${p.main}, ${p.accent})`,
                    outline: paletteId === p.id ? `3px solid #FBBF24` : '3px solid transparent',
            ...
            <div style={{ marginBottom: 32 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
                {FEATURES.map(f => (
                  <button key={f.id} onClick={() => setFeature(f.id)} style={{
                    ...choiceBtn,
                    flexDirection: 'column', gap: 4, padding: '12px 8px',
                    background: feature === f.id ? 'rgba(251,191,36,.15)' : 'rgba(255,255,255,.05)',
                    border: `1.5px solid ${feature === f.id ? '#FBBF24' : 'rgba(255,255,255,.1)'}`,
                  }}>
        ...
        {step === 3 && (
          <div>
            ...
            <p style={{ color: 'rgba(255,255,255,.4)', textAlign: 'center', fontSize: 14, marginBottom: 28 }}>
              Cela change comment il te parle
            </p>
            ...
            <div style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 20, padding: 24, marginBottom: 24, textAlign: 'center' }}>
              <p style={{ color: 'rgba(255,255,255,.5)', fontSize: 14, marginBottom: 16 }}>
            ...
            <button
              ...
              style={{
                ...btnStyle,
                background: palName.trim() ? '#FBBF24' : 'rgba(255,255,255,.1)',
                color: palName.trim() ? '#0B1F4B' : 'rgba(255,255,255,.3)',
        ...
        {step === 4 && (
          <div style={{ textAlign: 'center' }}>
            ...
            <h2 style={{
              fontFamily: 'var(--font-fredoka)', color: '#FBBF24',
            ...
            <div style={{
              ...
              transform: speaking ? 'translateY(0) scale(1)' : 'translateY(10px) scale(0.97)',
              ...
              <p style={{ color: 'rgba(255,255,255,.85)', fontSize: 16, lineHeight: 1.7, fontStyle: 'italic' }}>
                "{pers.greeting}"
              </p>
            </div>
            ...
            <button
              ...
              style={{
                ...btnStyle,
                background: saving ? 'rgba(255,255,255,.1)' : '#FBBF24',
                color: saving ? 'rgba(255,255,255,.3)' : '#0B1F4B',
                fontSize: 16, padding: '16px 48px',
              }}
            >
